...
const uri="mongodb+srv://nicolamontesanto1:tmvuVCsaFIWEMhAs@snmproject.ajdlcwk.mongodb.net/";
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (presente) {
      const passcheck = await bcrypt.compare(req.body.password, presente.password);
      if(passcheck){
...
async function finduser(mail) {
  try {
      ...
      const response = await fetch(`/finduser?email=${encodeURIComponent(mail)}`, {
          method: 'GET',
          headers: {
              'Content-Type': 'application/json'
          }
...
app.get('/finduser', async (req, res) => {
  try {
    ...
    const collection = db.collection('utenti');
    const presente = await collection.findOne({ email: email });
